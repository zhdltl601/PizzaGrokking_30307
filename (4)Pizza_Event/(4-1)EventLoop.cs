...
namespace _4_Pizza_Event
{
    internal class EventLoop
    {
        ...
        public void RunForever()
        {
            while (true)
            {
                List<Socket> readList = new List<Socket>(readers.Keys);     // 읽기 가능 소켓 목록
                List<Socket> writeList = new List<Socket>(writers.Keys);    // 쓰기 가능 소켓 목록
                Socket.Select(readList, writeList, null, 1000);             // 읽/쓸 수 있는 상태가 된 소켓 선별해 list에 반환

                foreach (Socket sock in readList)
                {
                    if (readers.TryGetValue(sock, out Action<Socket> callback))
                    {
                ...
                {
                    if (writers.TryGetValue(sock, out Action<Socket> callback))
                    {
